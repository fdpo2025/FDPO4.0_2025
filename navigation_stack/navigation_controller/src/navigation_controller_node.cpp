...
void NavigationController::followLine() {
    ...
    if (followLineFsm.state == navigation::followLineStates::Follow_Line) {
        ...
        double w_ratio = (param.w_nom > 1e-6) ? std::abs(w_d) / param.w_nom : 1.0;
        double w_ratio_cubed = w_ratio * w_ratio * w_ratio;
        v_d = std::max(param.approaching_vel * (1.0 - w_ratio_cubed), 0.0);
