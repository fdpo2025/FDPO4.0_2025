...
void NavigationController::buildSmoothedPathFromSegment() {
  ...
  size_t start_idx = 0;
  if (!route_seg_.empty() && route_seg_.front().backwards) {
      start_idx = 1;  // ignora o primeiro waypoint
  }

  for (size_t i = start_idx; i < route_seg_.size(); ++i) {
      const auto& wp = route_seg_[i];
      raw.push_back({wp.pose.x, wp.pose.y});
  }
...
void NavigationController::buildNextSegment() {
  ...
  ROS_INFO("---- Current Segment Waypoints ----");

  for (const auto& wp : route_seg_) {
    ROS_INFO("id=%d  x=%.3f  y=%.3f  backwards=%d",
            wp.id,
            wp.pose.x,
            wp.pose.y,
            wp.backwards ? 1 : 0);
