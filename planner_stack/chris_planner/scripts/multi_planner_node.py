...
class MultiPlannerNode:
    ...
    def __init__(self):
        ...
        self.robots = {
            "r1": {
                ...
                "goal_logical": None,
                "goal_physical": None,
                "path": [],                # path completo físico para reservas
                "compact_path": [],        # path reduzido físico para publicação
                "busy": False,
    ...
    def sequence_cb(self, msg):
        ...
        for robot_id in self.robots:
            ...
            self.robots[robot_id]["goal_logical"] = None
            self.robots[robot_id]["goal_physical"] = None
    ...
    def plan_for_robot(self, robot_id):
        ...
        robot_node_physical = r["current_node"]

        rospy.loginfo(f"[{robot_id}] planning from physical node={robot_node_physical}, logical state={state}")
        ...
        best_path = None
        best_goal_logical = None
        best_goal_physical = None
        best_cost = float("inf")
        ...
        for candidate_nodes in candidate_groups:
            ...
            local_best_goal_logical = None
            local_best_goal_physical = None
            local_best_cost = float("inf")

            for node_logical in candidate_nodes:
                if r["box"] == factory_module.EMPTY:
                    if not self.can_pickup_box_for_machine(
                        robot_id=robot_id,
                        pickup_node=node_logical,
                        ...
                        rospy.loginfo(f"[{robot_id}] pickup at node {node_logical} blocked by red machine capacity")
                        continue
                    ...
                    if not self.can_pickup_box_for_machine(
                        ...
                        rospy.loginfo(f"[{robot_id}] pickup at node {node_logical} blocked by green machine capacity")
                        continue

                node_physical = self.planner.resolve_physical_destination(robot_node_physical, node_logical)
                path = self.shortest_path_avoiding(robot_node_physical, node_physical, blocked_nodes)

                rospy.loginfo(
                    f"[{robot_id}] candidate logical_goal={node_logical}, "
                    f"physical_goal={node_physical}, avoided path={path}"
                )
                ...
                if cost < local_best_cost:
                    ...
                    local_best_goal_logical = node_logical
                    local_best_goal_physical = node_physical

            if local_best_path is not None:
                best_path = local_best_path
                best_goal_logical = local_best_goal_logical
                best_goal_physical = local_best_goal_physical
                best_cost = local_best_cost
                break

        rospy.loginfo(
            f"[{robot_id}] best_path={best_path}, "
            f"best_goal_logical={best_goal_logical}, best_goal_physical={best_goal_physical}"
        )
        ...
        if task_type == "pickup":
            ok = self.reserve_box_at_planning(robot_id, best_goal_logical)
            if not ok:
                rospy.logwarn(f"[{robot_id}] Failed to reserve box at node {best_goal_logical}")
                r["waiting_replan"] = True
        ...
        self.reserve_path(robot_id, full_path, best_goal_logical)
        self.publish_path(robot_id, compact_path)

        r["path"] = full_path
        r["compact_path"] = compact_path
        r["goal_logical"] = best_goal_logical
        r["goal_physical"] = best_goal_physical
        r["busy"] = True
        ...
        rospy.loginfo(
            f"{robot_id} task_type={task_type}, "
            f"goal_logical={best_goal_logical}, goal_physical={best_goal_physical}, cost={best_cost}"
        )
    ...
    def reserve_path(self, robot_id, path, goal_logical):
        ...
        self.reserved_goals[goal_logical] = robot_id
    ...
    def update_robot_position(self, robot_id, node):
        ...
        if r["goal_physical"] is not None and node == r["goal_physical"]:
            rospy.logwarn(
                f"[{robot_id}] GOAL DETECTED at physical node {node} "
                f"(logical goal={r['goal_logical']})"
            )
    ...
    def goal_reached(self, robot_id):
        ...
        r = self.robots[robot_id]
        goal_logical = r["goal_logical"]
        goal_physical = r["goal_physical"]
        task_type = r["task_type"]

        self.release_all_path_nodes_except_current(robot_id, goal_physical)

        rospy.loginfo(
            f"{robot_id} reached goal physical={goal_physical}, "
            f"logical={goal_logical}, task_type={task_type}"
        )

        if task_type == "pickup":
            state = (r["node"], r["box"], self.boxes)
            self.publish_state_snapshot(robot_id, "before pickup update", state)
            rospy.logwarn(f"[{robot_id}] BEFORE pickup update: state={state}, logical_goal={goal_logical}")

            new_state = self.factory.update_state(state, goal_logical)
            ...
            rospy.logwarn(
                f"[{robot_id}] AFTER pickup update: logical_node={r['node']} "
            ...
            rospy.logwarn(f"[{robot_id}] BEFORE dropoff update: state={state}, logical_goal={goal_logical}")
            ...
            rospy.logwarn(
                f"[{robot_id}] AFTER dropoff update: logical_node={r['node']} "
                f"box={r['box']} boxes={self.boxes}"
            )

        r["busy"] = False
        r["goal_logical"] = None
        r["goal_physical"] = None
        r["path"] = []
        r["compact_path"] = []
        r["task_type"] = None
    ...
    def try_replan_waiting_robot(self, freed_by_robot_id):
        ...
        if r["goal_logical"] is not None or r["goal_physical"] is not None:
            return
    ...
    def apply_output_warehouse_rule(self, valid_nodes):
        ...
        for other_id, other in self.robots.items():
            if (
                other["goal_logical"] is not None
                and other["task_type"] == "dropoff"
                and other["goal_logical"] in self.output_nodes
            ):
    ...
    def release_all_path_nodes_except_current(self, robot_id, current_node):
        ...
        goal_logical = r["goal_logical"]
        if goal_logical in self.reserved_goals and self.reserved_goals[goal_logical] == robot_id:
            del self.reserved_goals[goal_logical]
            rospy.loginfo(f"[{robot_id}] released current reserved logical goal {goal_logical} on goal arrival")
    ...
    def get_unavailable_pickup_nodes(self, robot_id):
        ...
        for other_id, other in self.robots.items():
            ...
            if other["goal_logical"] is not None and other["task_type"] == "pickup":
                blocked.add(other["goal_logical"])
